...
from image_recognition.utils import Demo
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run semantic segmentation on a video using DeepLabV3."
    )
    ...
    parser.add_argument(
        "--output",
        default="segmentation_output.mp4",
    ...
    demo = SegmentationDemo(device=device, alpha=args.alpha)
    ...
    progress = None if args.live else tqdm(total=total_frames, unit="frame")
    ...
    while True:
        ...
        if args.live:
            cv2.imshow("Semantic Segmentation Demo", output)
